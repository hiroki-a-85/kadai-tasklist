...
@section('content')
   ...
   <p id="show-status-label">Status:</p>
   <div id="show-status">{{ $task->status }}</div>
   <p id="show-content-label">Content:</p>
   <div id="show-content">{{ $task->content }}</div>
   
   <?php
   //strtr関数で'-'を'/'に置換
   //mb_substr関数で2020/04/03 19:23までを表示
